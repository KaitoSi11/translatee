...
label numbers_game:
    ...
    python:
        for i in range (1, renpy.random.randint (numBase, numLimit) ):
    ...
    python:
        for i in range (0, len(buttons_values) ):
            numbers_buttons.append ( {"b_number":i, "b_value":buttons_values[i], "b_x_pos":(renpy.random.randint (10, 182))*10, "b_y_pos":(renpy.random.randint (15, 93))*10, "b_to_show":True} )
    
    
    #"To win the game - click all the buttons one after another (start from \"1\")."
    "All I have to do is click through in the correct order."
    ...
    if result == "lose":
        ...
        #"You lose! Try again."
        #jump numbers_game
        if context == "E1D3S4_SystemCalibrationLoopback":
            jump E1D3S4_SystemCalibrationLoopback
